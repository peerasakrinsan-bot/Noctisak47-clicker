...
- **Metadata-driven (`VFX_MAP`):** each Elite/Mythic id maps to `{ rarity, theme, affects, aura, on, stack? }`:
  ...
  - `affects` — the real gameplay element the card changes (`odBar | combo | timer | zeny | break | enemy | player | debt`). On every (non-`hit`) trigger that element gets a short themed reaction via `targetPulse` (maps to `#godLevelWrap` / `#comboWrap` / `#timerDisplay` / `#scoreDisplay` / `#rageMeter` / `#boxer` / `#fighter` / `#debtStackCounter`).
