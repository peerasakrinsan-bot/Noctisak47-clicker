...
- **Metadata-driven (`VFX_MAP`):** each Elite/Mythic id maps to `{ rarity, theme, affects, aura, on, stack? }`:
  - `theme` — one of `soul | idol | analysis | crit | zeny | break | time | moonFever | darkJackpot | demonContract | debtContract | thanatos`; drives the `.game-vfx-theme-*` accent + effect personality. (`thanatos` is THANABROS's bespoke death / time-stop reaping identity — a death-violet/void-black/spectral-white `shadow` aura plus the `timeStop` / `voidRift` / `reaperScythe` / `deathKnell` / `soulReap` primitives — fired on THANATOS PHASE entry (`thanatos`: clock-stop freeze + void-gate rift + reaper-scythe sweep + tolling death-knell + souls reaped inward) and on AK47 WP hits during the phase (`ak47`: scythe sweep + souls reaped at the WP coord); `affects: timer` so the clock reacts since the card cuts/stops time.) (`moonFever` is MOONLIGHT FEVER's bespoke lunar-fever identity, built from the `moonPulse` / `crescentArc` / `eclipseRing` / `lunarSpark` / `feverWave` primitives. `darkJackpot` is DARK STAKE LORD's cursed-casino identity — a dark-gold `stake` aura plus the `slotReel` / `jackpotFlash` / `cursedCoin` / `stakeRing` / `suitSpark` / `riskPulse` primitives — fired from the BREAK reel-spin, the JACKPOT 777 payoff, and the no-win risk-warning flicker; `affects: zeny` so the Zeny/score HUD reacts, not just the card icon. `demonContract` is BAPHOBET's bespoke demon-contract / devil-bet identity — a blood-red/black breathing `infernal` aura plus the `demonSigil` / `contractRing` / `sinEmber` / `devilBetBurst` / `cursedFlame` / `bloodShock` primitives — fired from the BREAK DEVIL BET wager, each SIN gained (inward ember pull + sigil flash, with the real `_baphometSinStacks` 0–5 driving aura tier 1→3 via `ctx.tier`), and the `sinmax` CONTRACT SEALED payoff the moment stacks cap at 5; `affects: enemy` since the sin stacks curse damage onto the enemy. `debtContract` is LORD OF DEBT's bespoke debt-contract / inevitable-collection identity — a cursed-violet/ledger-gold breathing `debt` aura plus the `debtSeal` / `debtChain` / `ledgerGlyph` / `collectorPull` / `debtCoinDrain` / `sealBreak` primitives — fired from each DEBT CONTRACT signed (`debt`: seal stamp tinted by the active forbidden power via the `'$state'`→`ctx.color` sentinel, binding chains pulled inward, rising interest glyphs, with the real `_lodDebtStacks` 0–5 driving aura tier 1→3 via `ctx.tier`), the `debtmax` MAX-DEBT payoff the moment stacks newly hit 5 (collector gravity well + tight chains + dark flash), the `break` seal-shatter on any BREAK, and the `debtclear` collection (cursed coins siphoned away + aura tier reset to 0) when a BREAK actually clears owed stacks; `affects: debt` so the existing `#debtStackCounter` pulses in the freshly-signed power's `ctx.color`, not just the card icon.)
  - `affects` — the real gameplay element the card changes (`odBar | combo | timer | zeny | break | enemy | player | debt`). On every (non-`hit`) trigger that element gets a short themed reaction via `targetPulse` (maps to `#godLevelWrap` / `#comboWrap` / `#timerDisplay` / `#scoreDisplay` / `#rageMeter` / `#boxer` / `#fighter` / `#debtStackCounter`). `targetPulse` is throttled per target element (`_PULSE_THROTTLE = 90 ms`) so HUD pulses can't restart every frame during damage spam / simultaneous payoffs.
  ...
  - **Compact / passive feedback for non-pip cards** (no fake counters, all driven by real gameplay events):
    ...
    - **LORD OF DEBT** — `debtContract` theme; keeps its bespoke `#debtStackCounter` as the source of truth (no duplicate pip UI). The `debt` aura intensifies via `setAuraTier(id, 0–3)` from the real `_lodDebtStacks` 0–5 (accumulating obligation, visible passively between contracts), resetting to 0 on `debtclear`. Each DEBT CONTRACT stamps a `debtSeal` tinted by the active forbidden power (`'$state'`→`ctx.color`) + binding `debtChain` + `ledgerGlyph`; at MAX DEBT (newly 5) a `collectorPull` gravity well fires; BREAK shatters the seal (`sealBreak`) and, if stacks were owed, siphons `debtCoinDrain`. `affects: debt` makes the **existing** counter pulse in the freshly-signed power's color. BERSERK extra-hit gets a throttled chained shadow slash (`hit`).
...
- **Wiring** lives at existing mechanic hooks (BREAK success, AK47 complete, Overdrive start, Drake Take, Thanatos Phase, boss interactions, Doppelganger shadow strike, Abyssmell execute, MINORAGE ore gain, HYDRA head gain / HYDRA BURST, INCANTATION SCAMURAI CONTRACT open, LORD OF DEBT contract activation / MAX-debt / BREAK debt-clear / BERSERK shadow hit). Passive-damage cards get the aura only — **no per-hit particle spam** (mobile-safe brief).
...
- **Primitives supported on canvas:** flash, spark, slash, coinBurst, shadowBurst, fireBurst, bolt, breakCrack, streak, drainPulse, odGlow, bossFlare, pulse, comboRing, moonRing, holyBurst, glitch, the MOONLIGHT FEVER / DARK STAKE LORD / BAPHOBET bespoke sets, the LORD OF DEBT `debtContract` set (`debtSeal` / `debtChain` / `ledgerGlyph` / `collectorPull` / `debtCoinDrain` / `sealBreak`, draw kinds `dseal` / `dchain` / `dglyph` / `dcoin` + reused `void` / `ring` / `spark` / `bwave` / `crack`), the THANABROS `thanatos` set (`timeStop` / `voidRift` / `reaperScythe` / `deathKnell` / `soulReap`, new draw kinds `tclock` / `scythe` / `wisp` + reused `void` / `ring` / `glow` / `bwave` / `flash`), the fire-clone differentiators `clawRake` (EDGEGA tiger Lv2-Burst rake, draw kind `claw`) / `resonanceWave` (ATROSUS harmonic resonance, draw kind `rwave`) that move EDGEGA + ATROSUS off the shared `fireBurst` look so IFRIED alone owns the fire/inferno identity, and the unexpressed-fantasy set `insectSwarm` (BEELZEBRUH green fly swarm + MISSSTRESS gold bee queen, draw kind `swarm`) / `comboLock` (CATULLANUX cat-king combo lock on AK47-complete + BREAK, draw kind `lock`) / `voidZero` (COKE ZERO "ZERO" annihilation collapse on OD, draw kind `vzero`) / `corruptGlitch` (RSICK-0806 viral corruption blocks vs the shared scanline `glitch`, draw kind `cglitch`; FALLEN WECHAT stays on `glitch` but adds heavy `breakCrack` + `shadowBurst` + `bolt` for a system-overload-crash identity) — each kept visually distinct (color/shape/movement) to preserve per-card identity.
- **Safeguards:** global particle pool ceiling (`MAX_PARTICLES = 340`) plus a **dynamic particle budget** — the *effective* cap tightens under load (normal `320` / heavy combat `220` / Low VFX `140`) by sensing burst pressure (`≥ HEAVY_BURST_COUNT` bursts inside `HEAVY_BURST_WINDOW = 520 ms`) and retiring the **oldest** particles sooner, so simultaneous worst-case bursts (OD Lv3 + Mythic payoff + boss-skill VFX + BREAK) never pile up — an effect is never dropped, only the freshest survive. **Alpha-aware `shadowBlur`** (`_sb()`): the offscreen blur pass is skipped entirely once a stroke's alpha drops below `_SB_GATE` (0.25) and otherwise scaled by alpha (`blur = base × alpha`, clamped ≤ base), so faint fade-out frames pay no blur cost (eclipse blur base also trimmed 14→10; `suit` glyphs carry no blur — additive fill already pops); identical at full alpha. **Per-kind lifetime trim** (`_LIFE_TRIM`): monotonic-fade kinds (spark/ring/crack/coin/…) retire ~`life × 0.88–0.93`, i.e. once alpha has fallen below ~0.12 — visually-dead tail frames only, sin-fade/delayed kinds keep full life. Per-effect counts reduced under `prefers-reduced-motion`, DPR capped at 2.5, `requestAnimationFrame` loop **starts only when particles exist and stops when idle**, particles cleared + loop paused while the tab is hidden, and layout reads happen only in `resizeCanvasVfx()` (never inside the draw loop). `clearCanvasVfx()` is also called from `CardVFX.clearActive()` on run end.
