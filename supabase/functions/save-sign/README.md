...
หลัง deploy แล้วให้เช็คใน Supabase Dashboard > Edge Functions ว่ามี function ชื่อ `save-sign` และลองเปิด endpoint นี้ควรได้ response จาก function ไม่ใช่ 404:

```text
https://ouhtyyddclgqvrqqdwlc.supabase.co/functions/v1/save-sign
```

ถ้า deploy ผ่าน GitHub Actions/CI ต้องมี workflow ที่รัน `supabase functions deploy save-sign` ด้วย การ push ไฟล์ขึ้น GitHub อย่างเดียวไม่ deploy ให้อัตโนมัติ
